...
const APP_VERSION = '3.4.0-schema-adaptive';
...
// ==========================================================================
// ── Process-level crash guards: a single failed query must never
//    take the whole server down (was crashing on pool timeouts).
process.on('unhandledRejection', (reason) => { console.error('[UNHANDLED REJECTION]', (reason && reason.message) || reason); });
process.on('uncaughtException', (err) => { console.error('[UNCAUGHT EXCEPTION]', err && err.message || err); });

const pool = new Pool({
    connectionString: process.env.DATABASE_URL,
    ssl: { rejectUnauthorized: false },
    connectionTimeoutMillis: 20000,   // 20s to establish connection
    idleTimeoutMillis: 45000,         // Keep idle connections longer
    statement_timeout: 20000,         // 20s max per query
    max: 12,                          // Enough for the 11 parallel sync endpoints
...
const initDatabase = async () => {
    ...
    const migrations = [
        ...
        [`ALTER TABLE staff ADD COLUMN "passwordHash" TEXT;`, 'staff.passwordHash'],
        // FIXED (parent alerts): messages lost "category"/"studentId" on every
        // cloud sync (the table had no such columns), so attendance alerts
        // became unbound and showed up in EVERY parent's feed.
        [`ALTER TABLE messages ADD COLUMN category TEXT;`, 'messages.category'],
        [`ALTER TABLE messages ADD COLUMN "studentId" TEXT;`, 'messages.studentId'],
        // FIXED (finance): fee status lived only in the browser — these columns
        // let fee records persist and drive the finance widgets/reporting.
        [`ALTER TABLE students ADD COLUMN "feeStatus" TEXT;`, 'students.feeStatus'],
        [`ALTER TABLE students ADD COLUMN "feeBalance" TEXT;`, 'students.feeBalance'],
        [`ALTER TABLE users ADD COLUMN IF NOT EXISTS "studentId" TEXT;`, 'users.studentId'],
        [`ALTER TABLE staff ADD COLUMN IF NOT EXISTS "idNumber" TEXT;`, 'staff.idNumber'],
        [`ALTER TABLE staff ADD COLUMN IF NOT EXISTS "tscNumber" TEXT;`, 'staff.tscNumber'],
        [`ALTER TABLE students ADD COLUMN IF NOT EXISTS "nemisNumber" TEXT;`, 'students.nemisNumber'],
        [`ALTER TABLE students ADD COLUMN IF NOT EXISTS "upiNumber" TEXT;`, 'students.upiNumber'],
        [`UPDATE staff SET "passwordHash" = '${bcrypt.hashSync('teacher123', 10)}' WHERE "passwordHash" IS NULL;`, 'staff.passwordHashBackfill'],
    ...
    const tables = [
        ...
        [`staff`, `CREATE TABLE IF NOT EXISTS staff (
            ...
            phone TEXT, "tscNumber" TEXT, "idNumber" TEXT, photo TEXT, subjects TEXT, tsc TEXT, designation TEXT,
...
const seedDatabase = async () => {
    ...
    const usersToSeed = [
        ...
        { id: 'u4', email: 'deputy@school.com', name: 'Deputy Head Teacher', role: 'deputy_hoi', dept: 'Administration', pass: 'deputy123' },
        { id: 'u5', email: 'snr@school.com', name: 'Senior Teacher', role: 'snr_teacher', dept: 'Academic', pass: 'snr123' },
        { id: 'u6', email: 'chair@school.com', name: 'Exam Chair', role: 'exam_chair', dept: 'Exams', pass: 'chair123' },
        { id: 'u7', email: 'ct@school.com', name: 'Class Teacher', role: 'class_teacher', dept: 'Academic', pass: 'class123' },
        { id: 'u8', email: 'st@school.com', name: 'Subject Teacher', role: 'subject_teacher', dept: 'Academic', pass: 'subj123' }
            ...
            if (!dbUser && user.role === 'parent') {
                ...
                if (stu) {
                    dbUser = {
                        ...
                        role: 'parent', department: 'Student', isActive: 1, studentId: stu.id, parentMode: true
                    };
                }
            }
            // Staff-number sessions (teacher login via surname + TSC/ID)
            if (!dbUser && user.staffLogin) {
                const stRes = await pool.query('SELECT * FROM staff WHERE id = $1', [user.id]);
                const st = stRes.rows[0];
                if (st) {
                    dbUser = {
                        id: st.id, email: st.email || st.idNumber || st.tscNumber || st.id,
                        name: st.name, role: user.role, department: st.dept || 'Academic',
                        isActive: 1, staffMode: true
                    };
                }
            }
...
// Role alias groups — the granular 8-role model maps onto legacy guard lists.
// e.g. requireRole('teacher') also admits class_teacher & subject_teacher;
// requireRole('hoi') also admits deputy_hoi.
const ROLE_ALIASES = {
    admin: ['admin'],
    hoi: ['hoi', 'deputy_hoi'],
    deputy_hoi: ['hoi', 'deputy_hoi'],
    snr_teacher: ['teacher', 'snr_teacher'],
    exam_chair: ['exam_officer', 'exam_chair'],
    class_teacher: ['teacher', 'class_teacher'],
    subject_teacher: ['teacher', 'subject_teacher'],
    teacher: ['teacher', 'class_teacher', 'subject_teacher', 'snr_teacher'],
    exam_officer: ['exam_officer', 'exam_chair'],
    parent: ['parent']
};
const ROLE_LABELS_SRV = {
...
const requireRole = (...roles) => (req, res, next) => {
    ...
    const ok = roles.some(r => req.user.role === r || (ROLE_ALIASES[r] || []).includes(req.user.role));
    if (!ok) {
        const required = roles.map(r => ROLE_LABELS_SRV[r] || r).join(' or ');
        const current = ROLE_LABELS_SRV[req.user.role] || req.user.role;
...
//   AUTH ROUTES
// ==========================================================================
// ── Schema-aware lookups: detect which identifier columns actually exist,
//    so login/signup never 500s on an older database. ──
let _colCache = null;
async function getColumns(table) {
    ...
    } catch (_) { _colCache[table] = new Set(); }
    return _colCache[table];
}
async function studentLookupByAdm(adm) {
    const cols = await getColumns('students');
    const parts = ['LOWER(reg) = $1'];
    if (cols.has('nemisnumber')) parts.push('LOWER("nemisNumber") = $1');
    if (cols.has('upinumber')) parts.push('LOWER("upiNumber") = $1');
    return (await pool.query(`SELECT * FROM students WHERE ${parts.join(' OR ')}`, [String(adm).trim().toLowerCase()])).rows;
}
async function staffLookupByNumber(number) {
    const cols = await getColumns('staff');
    const parts = [];
    if (cols.has('tscnumber')) parts.push('LOWER("tscNumber") = $1');
    if (cols.has('idnumber')) parts.push('LOWER("idNumber") = $1');
    if (!parts.length) return [];
    return (await pool.query(`SELECT * FROM staff WHERE ${parts.join(' OR ')}`, [String(number).trim().toLowerCase()])).rows;
}

// helpers: staff surname (last word of full name) + student name matching
const staffSurname = (fullName) => { const parts = String(fullName || '').trim().split(/\s+/); return parts.length ? parts[parts.length - 1].toLowerCase() : ''; };
const normName = (n) => String(n || '').toLowerCase().replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();
const studentNameMatches = (recordName, input) => {
    const a = normName(recordName), b = normName(input);
    if (!a || !b) return false;
    if (a === b) return true;
    const words = b.split(' ');
    const first = words[0], last = words[words.length - 1];
    return first && last && a.includes(first) && a.includes(last);
};
const roleFromPosition = (position) => {
    const p = String(position || '').toLowerCase();
    if (/deputy|assist/.test(p)) return 'deputy_hoi';
    if (/head|principal|hoi/.test(p)) return 'hoi';
    if (/senior|snr/.test(p)) return 'snr_teacher';
    if (/exam/.test(p)) return 'exam_chair';
    if (/class/.test(p)) return 'class_teacher';
    return 'subject_teacher';
};

app.post('/api/login', rateLimit({ windowMs: 60 * 60 * 1000, max: 15 }), async (req, res) => {
    try {
        const { email, password, loginMode, studentName, studentAdm, staffSurname, staffNumber } = req.body;
        if (!password) return res.status(400).json({ success: false, message: 'Password is required.' });
        const identifier = String(email || '').trim().toLowerCase();
        const mode = loginMode || 'auto';

        // ── TEACHER LOGIN VIA SURNAME + TSC/ID NO ──
        if (mode === 'staff') {
            const surname = String(staffSurname || '').trim();
            const number = String(staffNumber || '').trim();
            if (!surname || !number) return res.status(400).json({ success: false, message: 'Enter your surname and TSC / ID number.' });
            const st = (await staffLookupByNumber(number)).find(r => staffSurname(r.name) === surname.toLowerCase());
            if (!st) return res.status(401).json({ success: false, message: 'No staff record matches that surname and TSC / ID number.' });
            if (!st.passwordHash) return res.status(401).json({ success: false, message: 'No login password set for this staff record. Use your email login or ask Admin to reset it.' });
            if (!bcrypt.compareSync(password, st.passwordHash)) return res.status(401).json({ success: false, message: 'Invalid credentials.' });
            // Resolve role: an existing linked user account wins; otherwise provision from position
            const linkRes = await pool.query('SELECT * FROM users WHERE LOWER(email) = $1 OR LOWER(name) = $2',
                [String(st.email || '').toLowerCase(), String(st.name || '').toLowerCase()]);
            const account = linkRes.rows[0] || { id: st.id, email: st.email || (st.tscNumber || st.idNumber) + '@staff.local', name: st.name, role: roleFromPosition(st.position) };
            const token = jwt.sign({ id: st.id, email: account.email, role: account.role, staffLogin: true }, JWT_SECRET, { expiresIn: '7d' });
            await logAction(st.id, st.name, 'LOGIN', `Staff login via TSC/ID from ${req.ip}`);
            return res.json({
                success: true, token,
                user: { id: st.id, email: account.email, role: account.role, name: st.name, department: st.dept || 'Academic', staffLogin: true }
            });
        }

        // ── PARENT LOGIN VIA STUDENT NAME + ADMISSION NO ──
        if (mode === 'parent') {
            const sName = String(studentName || '').trim();
            const sAdm = String(studentAdm || '').trim().toLowerCase();
            if (!sName || !sAdm) return res.status(400).json({ success: false, message: 'Enter the student name and admission number.' });
            const stu = (await studentLookupByAdm(sAdm)).find(r => studentNameMatches(r.name, sName));
            if (!stu) return res.status(401).json({ success: false, message: 'No learner matches that name and admission number. These can be changed by the school later.' });
            ...
            await logAction(stu.id, stu.name, 'LOGIN', `Parent login via name + ${stu.reg} from ${req.ip}`);
            return res.json({
                success: true, token,
        ...
        // ── PARENT ACCESS VIA STUDENT CREDENTIALS (ADM-only fallback) ──
        if (!user) {
            const stu = (await studentLookupByAdm(identifier))[0];
            if (stu) {
                if (!stu.passwordHash) return res.status(401).json({ success: false, message: 'No access password set for this learner. Contact the school.' });
                if (!bcrypt.compareSync(password, stu.passwordHash)) return res.status(401).json({ success: false, message: 'Invalid credentials.' });
                const token = jwt.sign({ id: stu.id, email: stu.reg, role: 'parent' }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/api/signup', rateLimit({ windowMs: 60 * 60 * 1000, max: 5 }), async (req, res) => {
    try {
        const { name, email, password, role, linkAdm, linkGuardianPhone } = req.body;
        ...
        // ── OPTIONAL PARENT ↔ STUDENT LINKING DURING SIGNUP ──
        // The parent enters the learner's ADM / NEMIS / UPI number plus the
        // guardian phone number on file. We verify both, then store the
        // learner's id on the new account (users.studentId) — the /api/db
        // parent branch and the Parent Portal then resolve the child from it.
        let studentId = null, linkedStudent = null;
        if (assignedRole === 'parent' && (linkAdm || linkGuardianPhone)) {
            if (!linkAdm || !linkGuardianPhone) {
                return res.status(400).json({ success: false, message: 'To link your child, enter BOTH the learner ADM/NEMIS/UPI and the guardian phone number on file.' });
            }
            const adm = String(linkAdm).trim().toLowerCase();
            const st = (await studentLookupByAdm(adm))[0];
            if (!st) return res.status(400).json({ success: false, message: 'No learner matches that ADM/NEMIS/UPI number. Double-check and try again (linking is optional).' });
            const norm = (p) => String(p || '').replace(/\D/g, '');
            if (!st.guardianPhone || norm(st.guardianPhone) !== norm(linkGuardianPhone)) {
                return res.status(400).json({ success: false, message: 'The guardian phone number does not match the one on file for this learner. Contact the school if this is wrong (linking is optional).' });
            }
            const already = await pool.query('SELECT id FROM users WHERE "studentId" = $1', [st.id]);
            if (already.rows.length > 0) {
                return res.status(400).json({ success: false, message: 'This learner is already linked to a parent account. Use the existing account or ask the school to re-link it.' });
            }
            studentId = st.id;
            linkedStudent = st;
        }
        ...
        if (userCols.has('studentid')) {
            await pool.query('INSERT INTO users (id, email, name, role, "department", "passwordHash", "studentId") VALUES ($1,$2,$3,$4,$5,$6,$7)',
                [Date.now().toString(), email, name, assignedRole, 'General', bcrypt.hashSync(password, 10), studentId]);
        } else {
            console.warn('[SCHEMA] users.studentId missing — run migrations (restart) to enable parent linking.');
            await pool.query('INSERT INTO users (id, email, name, role, "department", "passwordHash") VALUES ($1,$2,$3,$4,$5,$6)',
                [Date.now().toString(), email, name, assignedRole, 'General', bcrypt.hashSync(password, 10)]);
        }
        await logAction('system', 'System', 'SIGNUP_REQUEST', `${name} (${email}) requested ${assignedRole} access${linkedStudent ? ' — linked to learner ' + linkedStudent.name + ' (' + linkedStudent.grade + ')' : ''}`);
        res.status(201).json({
            success: true,
            message: linkedStudent
                ? `Account created and linked to ${linkedStudent.name} (${linkedStudent.grade}). Sign in with your email and password.`
...
app.post('/api/change-password', authenticateToken, async (req, res) => {
    try {
        ...
        if (req.user.staffMode) {
            await pool.query('UPDATE staff SET "passwordHash" = $1 WHERE id = $2', [hash, req.user.id]);
        } else if (req.user.parentMode) {
            await pool.query('UPDATE students SET "passwordHash" = $1 WHERE id = $2', [hash, req.user.id]);
        } else {
            await pool.query('UPDATE users SET "passwordHash" = $1 WHERE id = $2', [hash, req.user.id]);
        }
...
// ==========================================================================
//   PARENT SELF-LINK — link this parent account to a learner (ADM + phone)
// ==========================================================================
app.post('/api/link-student', authenticateToken, async (req, res) => {
    try {
        const { adm, guardianPhone } = req.body;
        if (req.user.role !== 'parent') return res.status(403).json({ success: false, message: 'Only parent accounts can link a learner.' });
        // Student-backed sessions (signed in via ADM + access password) are already linked
        if (req.user.parentMode) {
            const stu = (await pool.query('SELECT id, name, grade FROM students WHERE id = $1', [req.user.id])).rows[0];
            return res.json({ success: true, alreadyLinked: true, student: stu || null });
        }
        if (!adm || !guardianPhone) return res.status(400).json({ success: false, message: 'Enter BOTH the learner ADM / NEMIS / UPI and the guardian phone number on file.' });
        const stu = (await studentLookupByAdm(adm))[0];
        if (!stu) return res.status(400).json({ success: false, message: 'No learner matches that ADM / NEMIS / UPI number. Double-check and try again.' });
        const norm = (p) => String(p || '').replace(/\D/g, '');
        if (!stu.guardianPhone || norm(stu.guardianPhone) !== norm(guardianPhone)) {
            return res.status(400).json({ success: false, message: 'The guardian phone number does not match the one on file for this learner. Contact the school if this is wrong.' });
        }
        const already = (await pool.query('SELECT id FROM users WHERE "studentId" = $1 AND id <> $2', [stu.id, req.user.id])).rows[0];
        if (already) return res.status(400).json({ success: false, message: 'This learner is already linked to another parent account. Ask the school to re-link it.' });
        await pool.query('UPDATE users SET "studentId" = $1 WHERE id = $2', [stu.id, req.user.id]);
        await logAction(req.user.id, req.user.name || 'Parent', 'LINK_STUDENT', `Linked to learner ${stu.name} (${stu.grade})`);
        broadcastChange();
        res.json({ success: true, student: { id: stu.id, name: stu.name, grade: stu.grade } });
    } catch (err) {
        console.error('[LINK STUDENT ERROR]', err);
        res.status(500).json({ success: false, message: 'Linking failed. Try again or contact the school.' });
    }
});
...
// ==========================================================================
const USER_ROLES = ['admin', 'hoi', 'deputy_hoi', 'snr_teacher', 'exam_chair', 'class_teacher', 'subject_teacher', 'teacher', 'exam_officer', 'parent'];
...
app.post('/api/students/:id/password', authenticateToken, requireRole('admin', 'hoi'), async (req, res) => {
    ...
    if (strength.length) return res.status(400).json({ success: false, message: 'Password needs: ' + strength.join(', ') });
    try {
        const stu = (await pool.query('SELECT * FROM students WHERE id = $1', [req.params.id])).rows[0];
        ...
        res.json({
            ...
            login: { reg: stu.reg, nemis: stu.nemisNumber || null, upi: stu.upiNumber || null }
        });
...
// FIXED (persistence): a plain DELETE + INSERT dropped every row's
// "passwordHash" / "parentPasswordSet" (parent-login credentials) on each
// cloud sync — the incoming payload does not carry them, so parents were
// silently locked out after any student/staff edit. Now: delete only rows
// that are no longer present, then UPSERT the rest so sensitive columns
// are never touched.
const replacePreserving = async (client, table, rows, cols) => {
    const ids = rows.map(r => r.id).filter(Boolean);
    // Remove records the client deleted (chunked to stay under the 65535-param cap)
    for (let i = 0; i < ids.length; i += 500) {
        const chunk = ids.slice(i, i + 500);
        const ph = chunk.map((_, j) => `$${j + 1}`).join(',');
        await client.query(`DELETE FROM "${table}" WHERE id NOT IN (${ph})`, chunk);
    }
    // Upsert each row — sensitive columns absent from `cols` survive untouched
    ...
    for (const i of rows) {
        ...
        await client.query(`INSERT INTO "${table}" ("${cols.join('","')}") VALUES (${placeholders}) ON CONFLICT (id) DO UPDATE SET ${updateSet}`, values);
    }
};

app.post('/students', authenticateToken, requireRole('hoi', 'admin'), expectArray, async (req, res) => {
    const cols = ['id','name','gender','dob','idNumber','phone','grade','stream','reg','photo','guardianName','guardianPhone','guardianRel','upiNumber','prevSchool','entryLevel','yearCompleted','nemisNumber','disability','feeStatus','feeBalance'];
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await replacePreserving(client, 'students', req.body, cols);
        await client.query('COMMIT');
        await logAction(req.user.id, req.user.name, 'UPDATE_STUDENTS', `${req.body.length} records`);
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[STUDENTS POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
...
app.post('/staff', authenticateToken, requireRole('hoi', 'admin'), expectArray, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        await replacePreserving(client, 'staff', req.body, cols);
        await client.query('COMMIT');
        await logAction(req.user.id, req.user.name, 'UPDATE_STAFF', `${req.body.length} records`);
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[STAFF POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
    finally { client.release(); }
});
...
app.post('/learningAreas', authenticateToken, requireRole('hoi', 'admin'), expectArray, async (req, res) => {
    ...
    try {
        ...
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[AREAS POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
    finally { client.release(); }
});
...
app.post('/notes', authenticateToken, requireRole('admin', 'hoi'), expectArray, async (req, res) => {
    ...
    try {
        ...
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[NOTES POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
    finally { client.release(); }
});
...
app.post('/timetable', authenticateToken, requireRole('admin', 'hoi'), expectArray, async (req, res) => {
    ...
    try {
        ...
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[TIMETABLE POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
    finally { client.release(); }
});
...
app.post('/messages', authenticateToken, expectArray, async (req, res) => {
    // FIXED: category + studentId added so attendance alerts stay bound to
    // the learner they concern (previously dropped on every sync).
    const cols = ['id','to','from','subject','body','date','read','folder','category','studentId'];
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        ...
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[MESSAGES POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
    finally { client.release(); }
});
...
app.post('/classAssignments', authenticateToken, requireRole('hoi', 'admin'), expectArray, async (req, res) => {
    ...
    try {
        ...
        broadcastChange();
        res.json(req.body);
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[CLASS ASSIGN POST ERROR]', err); res.status(500).json({ error: 'DB Error', details: err.message }); }
    finally { client.release(); }
});
...
app.post('/api/exams/sync', authenticateToken, requireRole('exam_officer', 'hoi', 'admin', 'teacher'), async (req, res) => {
    ...
    } catch (err) { try { await client.query('ROLLBACK'); } catch (_) {} console.error('[EXAMS SYNC ERROR]', err); res.status(500).json({ error: 'Exam sync failed', details: err.message }); }
    finally { client.release(); }
});
...
// Every connected tab refreshes ~1-2s after any data change (any device/user).
...
app.get('/api/events', (req, res) => {
    const token = String(req.query.token || '').split(' ').pop();
    ...
    res.write(': connected\n\n');
    sseClients.add(res);
    const keep = setInterval(() => { try { res.write(': keep\n\n'); } catch (_) { clearInterval(keep); sseClients.delete(res); } }, 20000);
    req.on('close', () => { clearInterval(keep); sseClients.delete(res); });
});

app.get('/api/version', (req, res) => res.json({ version: APP_VERSION, name: 'ElimuTrack API', status: 'ok' }));
...
app.get('/api/db', authenticateToken, requireRole('admin', 'hoi', 'teacher', 'exam_officer', 'parent'), async (req, res) => {
    try {
        ...
        if (req.user.role === 'parent') {
            ...
            // FIXED (parent assessments): include every assessment for the
            // child — either by grade match, or any wrapper that holds scores
            // keyed to the linked learner (covers empty/legacy grade fields).
            const gradeExams = parsedExams.filter(e => gradeSet.has(e.grade) ||
                (e.scores && typeof e.scores === 'object' && linked.some(l => Object.keys(e.scores).some(k => k.startsWith(l.id + '_')))));
            res.json({
                students: linked,
...
app.post('/api/restore', authenticateToken, requireRole('admin', 'hoi'), async (req, res) => {
    ...
    // FIXED (persistence): `preserveCols` — columns whose existing DB values
    // are merged into the incoming rows when the backup omits them. This keeps
    // parent-login (students.passwordHash / parentPasswordSet) and staff-login
    // (staff.passwordHash) credentials alive across a restore; a plain
    // DELETE + INSERT used to silently wipe every login.
    const safeReplace = async (client, table, data, columns, preserveCols = []) => {
        if (!data || !Array.isArray(data) || data.length === 0) return;

        if (preserveCols.length > 0) {
            const ids = data.map(r => r.id).filter(Boolean);
            const existing = new Map();
            for (let i = 0; i < ids.length; i += 500) {
                const chunk = ids.slice(i, i + 500);
                const ph = chunk.map((_, j) => `$${j + 1}`).join(',');
                const q = `SELECT id, ${preserveCols.map(c => `"${c}"`).join(', ')} FROM "${table}" WHERE id IN (${ph})`;
                const r = await client.query(q, chunk);
                r.rows.forEach(row => existing.set(row.id, row));
            }
            data = data.map(r => {
                const ex = existing.get(r.id);
                if (!ex) return r;
                const copy = { ...r };
                preserveCols.forEach(c => {
                    if (copy[c] === null || copy[c] === undefined || copy[c] === '') copy[c] = ex[c];
                });
                return copy;
            });
        }
    ...
    try {
        ...
        await safeReplace(client, 'students', students, ['id','name','gender','dob','idNumber','phone','grade','stream','reg','photo','guardianName','guardianPhone','guardianRel','upiNumber','prevSchool','entryLevel','yearCompleted','nemisNumber','disability','passwordHash','parentPasswordSet'], ['passwordHash','parentPasswordSet']);
        await safeReplace(client, 'staff', staff, ['id','name','email','role','department','phone','tscNumber','photo','subjects','tsc','designation','idNo','dob','gender','employmentType','appointmentDate','surname','firstName','otherNames','dept','passwordHash'], ['passwordHash']);
        ...
        await safeReplace(client, 'learningAreas', learningAreas, ['id','name','code','applicableLevels','teacherId','teacherByLevel']);
        ...
        await safeReplace(client, 'messages', messages, ['id','to','from','subject','body','date','read','folder','category','studentId']);
...
// ── Health check (for load balancers / uptime monitors / docker healthchecks) ──
app.get('/api/health', async (req, res) => {
    try {
        await pool.query('SELECT 1');
        res.json({ ok: true, version: APP_VERSION, db: 'connected', uptime: Math.round(process.uptime()) });
    } catch (e) {
...
initDatabase().then(() => {
    console.log('✅ ElimuTrack API v' + APP_VERSION + ' starting — http://localhost:' + PORT + '\n');
